"""
Script to clear all data and seed real data from frontend
"""
...
print("Clearing all existing locations...")
db.campus_locations.delete_many({})

# Real locations from frontend (only Tewodros campus - 6 categories)
# Categories: building, administration, library, lab, cafe, dorm
real_locations = [
...
print(f"Inserting {len(real_locations)} real locations...")
result = db.campus_locations.insert_many(real_locations)
print(f"Inserted {len(result.inserted_ids)} locations")

# Verify
print("\nVerifying data...")
total = db.campus_locations.count_documents({})
print(f"Total locations in database: {total}")

# Count by campus
for campus in ['tewodros', 'maraki', 'fasil']:
    count = db.campus_locations.count_documents({'campus': campus})
    print(f"  - {campus}: {count} locations")

# Count by category
categories = ['building', 'administration', 'library', 'lab', 'cafe', 'dorm']
print("\nLocations by category:")
for cat in categories:
    count = db.campus_locations.count_documents({'category': cat})
    print(f"  - {cat}: {count}")

print("\nDone! Real data has been seeded.")
